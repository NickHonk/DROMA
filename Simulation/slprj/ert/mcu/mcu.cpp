               ...
               *rty_led)
{
  ...
  tmp_6 = _mm_add_pd(_mm_sub_pd(_mm_loadu_pd(&rtu_Bus_IMU_imu_gyro[0]),
  ...
  if (na > 0.5) {
    ...
    tmp_7 = _mm_div_pd(_mm_loadu_pd(&rtu_Bus_Cmd_q_ext[0]), tmp_6);
    _mm_storeu_pd(&tmp_4[0], tmp_7);
    na = tmp_4[0];
    rtb_q_des_idx_1 = tmp_4[1];
    tmp_6 = _mm_div_pd(_mm_loadu_pd(&rtu_Bus_Cmd_q_ext[2]), tmp_6);
    _mm_storeu_pd(&tmp_4[0], tmp_6);
  ...
  theta[2] = ((25.0 * rtb_tau_ref[2] + theta[2]) + (rtu_Bus_IMU_imu_gyro[2] -
    0.17453292519943295)) * 0.001;
  q_err_idx_1 = norm_XZxrXx9N(theta);
  ...
  tmp_6 = _mm_set1_pd(norm_bQtbKOLy(mcu_DW.q));
  ...
  if (mcu_B.RateTransition) {
    ...
    q_err_idx_1 = *rtu_Bus_Cmd_F_des;
    na = rtu_Bus_Cmd_q_des[0];
    q_err_idx_2 = rtu_Bus_Cmd_q_ref[0];
    rtb_q_des_idx_1 = rtu_Bus_Cmd_q_des[1];
    q_err_idx_3 = rtu_Bus_Cmd_q_ref[1];
    rtb_q_des_idx_2 = rtu_Bus_Cmd_q_des[2];
    rtb_q_ref_idx_2 = rtu_Bus_Cmd_q_ref[2];
  ...
  if (mcu_DW.latched) {
    ...
    for (i = 0; i <= 0; i += 2) {
      ...
      _mm_storeu_pd(&rtb_tau_ref_0[i], _mm_sub_pd(tmp_6, _mm_add_pd(_mm_add_pd
        (_mm_mul_pd(_mm_loadu_pd(&rtCP_Constant_Value_m[i + 3]), _mm_set1_pd
                    (rtb_q_des_idx_1)), _mm_mul_pd(_mm_loadu_pd
        (&rtCP_Constant_Value_m[i]), _mm_set1_pd(na))), _mm_mul_pd(_mm_loadu_pd(
        &rtCP_Constant_Value_m[i + 6]), _mm_set1_pd(rtb_q_des_idx_2)))));
      _mm_storeu_pd(&tmp_2[i], _mm_set1_pd(0.0));
    }

    for (i = 2; i < 3; i++) {
      _mm_storeu_pd(&tmp_4[0], _mm_sub_pd(_mm_set_pd(rtb_tau_ref[i], rtb_Add[i]),
        _mm_add_pd(_mm_add_pd(_mm_mul_pd(_mm_set_pd(rtCP_Constant_Value_m[i + 3],
        S_tmp_0[i + 3]), _mm_set_pd(rtb_q_des_idx_1, rtb_q_des_idx_3)),
        _mm_mul_pd(_mm_set_pd(rtCP_Constant_Value_m[i], S_tmp_0[i]), _mm_set_pd
                   (na, q_err_idx_2))), _mm_mul_pd(_mm_set_pd
        (rtCP_Constant_Value_m[i + 6], S_tmp_0[i + 6]), _mm_set_pd
        (rtb_q_des_idx_2, q_err_idx_3)))));
      theta[i] = tmp_4[0];
    ...
    for (i = 0; i < 3; i++) {
      _mm_storeu_pd(&tmp_4[0], _mm_add_pd(_mm_mul_pd(_mm_loadu_pd
        (&rtCP_Constant1_Value_o[3 * i]), _mm_set1_pd(theta[i])), _mm_set_pd
        (q_err_idx_2, R_tmp)));
      R_tmp = tmp_4[0];
      q_err_idx_2 = tmp_4[1];
      q_err_idx_3 += rtCP_Constant1_Value_o[3 * i + 2] * theta[i];
    }
    ...
    for (i = 0; i < 4; i++) {
      ...
      _mm_storeu_pd(&tmp_4[0], _mm_add_pd(_mm_mul_pd(_mm_loadu_pd
        (&rtCP_Gain_Gain[i_0]), tmp_6), _mm_set_pd(rtb_q_des_idx_1, na)));
      na = tmp_4[0];
      rtb_q_des_idx_1 = tmp_4[1];
      _mm_storeu_pd(&tmp_4[0], _mm_add_pd(_mm_mul_pd(_mm_loadu_pd
        (&rtCP_Gain_Gain[i_0 + 2]), tmp_6), _mm_set_pd(q_err_idx_1,
        rtb_q_des_idx_2)));
      rtb_q_des_idx_2 = tmp_4[0];
      q_err_idx_1 = tmp_4[1];
    }
    ...
    rty_rotor_cmd[3] = std::sqrt(std::abs(q_err_idx_1));
  }
